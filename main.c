...
static void init_charset (void) {/*{{{*/
  ...
  char *lang = getenv("LANG") ? g_ascii_strup(getenv("LANG"), -1) : NULL;
  char *lc_ctype = getenv("LC_CTYPE") ? g_ascii_strup(getenv("LC_CTYPE"), -1) : NULL;
...
static char * normalize_wm_state_name(const char * name)
{
    ...
    for (i = 0; short_names[i]; i++)
    {
        ...
        {
            char * upcase = g_ascii_strup(name, -1);
            char * result = g_strdup_printf("_NET_WM_STATE_%s", upcase);
            g_free(upcase);
            return result;
        }
    }
...
static int window_say_title (Display *disp, Window win) {
    char *title_utf8 = get_window_title(disp, win);
    printf("%s\n", title_utf8);
    g_free(title_utf8);
    return EXIT_SUCCESS;
}
